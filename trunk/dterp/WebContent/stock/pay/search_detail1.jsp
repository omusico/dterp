...
<script type="text/javascript" src="../../javascript/include/validate/validation-framework.js"></script>
 ...
 <td <%=TD_HANDBOOK_STYLE1%> class="TD_HANDBOOK_STYLE1"><div class="div_handbook"><%="您正在做的业务是：库存管理--出库管理--出库查询"%></div></td>
 </tr>
 </table>
 <%
 //HttpSession dbSession = request.getSession();// 获取会话session
	...
	int proId = Integer.parseInt(request.getParameter("proId"));//出库理由ID
	
	if(stock_db1.conn((String)session.getAttribute("unit_db_name"))){
	
	String sql = "select * from stock_out inner join hr_file on hr_file.idcard=stock_out.stock_out_operator_id where stock_out.id = "+id;
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 ...
 <table <%=TABLE_STYLE4%> class="TABLE_STYLE4" id=theObjTable>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="11%"><%=demo.getLang("erp","出库单号")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%" style="text-align:left"><input type="text" <%=INPUT_STYLE3%> readonly="readonly" class="INPUT_STYLE3" name="gatherer_name" value="<%=rs.getString("stock_out.stock_out_id")  %>"></td>
   <td <%=TD_STYLE1%> class="TD_STYLE8" width="11%"><%=demo.getLang("erp","出库申请单号")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%" style="text-align:left"><input type="text" <%=INPUT_STYLE3%> class="INPUT_STYLE3" readonly="readonly" name="gatherer_name" value="<%=request.getParameter("audOrder") %>"></td>
  </tr>
  <tr>
  <td <%=TD_STYLE1%> class="TD_STYLE8" width="11%">出库人：</td><td <%=TD_STYLE2%> class="TD_STYLE2" width="40%" style="text-align:left"><input type="text" readonly="readonly" <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="gatherer_name" value="<%=rs.getString("hr_file.human_name") %>"></td>
   
   <td <%=TD_STYLE1%> class="TD_STYLE8" width="11%">出库理由：</td>
   <%if(proId==3){ %>
   <td><input type="text" <%=INPUT_STYLE3%> readonly="readonly" class="INPUT_STYLE3" name="gatherer_name"  value="原材料出库" /></td>
  <%}else if(proId==5){ %>
  <td><input type="text" <%=INPUT_STYLE3%> readonly="readonly" class="INPUT_STYLE3" name="gatherer_name"  value="成品出库" /></td>
  <%} %>
  </tr>
...
<%if(proId==3){ %>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="10%"><%=demo.getLang("erp","出库时间")%></td>
<td <%=TD_STYLE2%> class="TD_STYLE2" width="10%"><%=demo.getLang("erp","库位")%></td>
<td <%=TD_STYLE2%> class="TD_STYLE2" width="10%"><%=demo.getLang("erp","出库状态")%></td>
 </tr>
 <%
	sql = "select * from stock_out_detail inner join product_info on product_info.id =stock_out_detail.out_Detail_product_id   where stoct_out_id='"+rs.getString("id")+"'";
 	ResultSet rs45 = stock_db1.executeQuery(sql);
 	while(rs45.next()){
 		String time1 = rs.getString("stock_out_time");
...
<tr <%=TR_STYLE1%> class="TR_STYLE1" >
 ...
 <input <%=INPUT_STYLE4%> value="<%=rs45.getString("out_Detail_invoice_no") %>" class="INPUT_STYLE4" name="product_name" type="text" onFocus="this.blur()">
 </td>
 <td <%=TD_STYLE2%> class="TD_STYLE2"><input <%=INPUT_STYLE4%> value="<%=rs45.getString("out_Detail_width") %>" class="INPUT_STYLE4" name="product_name" type="text" onFocus="this.blur()"></td>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><input <%=INPUT_STYLE4%> value="<%=time1 %>" class="INPUT_STYLE4" name="product_name" type="text" onFocus="this.blur()"></td>
 <td <%=TD_STYLE2%> class="TD_STYLE2"><input <%=INPUT_STYLE4%> class="INPUT_STYLE4" name="cost_price" type="text"  onFocus="this.blur()" value=""></td>
 
 <td <%=TD_STYLE2%> class="TD_STYLE2"><input <%=INPUT_STYLE4%> value="已出库" class="INPUT_STYLE4" name="product_name" type="text" onFocus="this.blur()"></td>
 </tr>
 <% }%>
</table>
<%}else if(proId==5){ %>
<table <%=TABLE_STYLE5%> class="TABLE_STYLE5" cols=1 id=tableOnlineEdit1
	...
	<thead>
		...
		<%
					String sql_t = "select * from stock_out_detail inner join package_info on package_info.id =stock_out_detail.out_Detail_product_id inner join crm_file on crm_file.id =stock_out_detail.Out_Detail_custom_id   where stoct_out_id='"+ rs.getString("id") + "'";
		...
		<tr <%=TR_STYLE1%> class="TR_STYLE1">
			...
			<td <%=TD_STYLE2%> class="TD_STYLE2"><input <%=INPUT_STYLE4%>
				class="INPUT_STYLE4" name="cost_price" type="text"
				onFocus="this.blur()" value="<%=rs45.getString("CUSTOMER_NAME") %>"></td>
 <td <%=TD_STYLE2%> class="TD_STYLE2"><input <%=INPUT_STYLE4%> value="<%=rs.getString("stock_out_time") %>" class="INPUT_STYLE4" name="product_name" type="text" onFocus="this.blur()"></td>
 <td <%=TD_STYLE2%> class="TD_STYLE2"><input <%=INPUT_STYLE4%> class="INPUT_STYLE4" name="cost_price" type="text"  onFocus="this.blur()" value=""></td>
...
<textarea <%=TEXTAREA_STYLE1%> class="TEXTAREA_STYLE1" name="remark" readonly="readonly"><%=rs.getString("stock_out_remark") %></textarea>
 ...
 <input type="text" readonly="readonly" <%=INPUT_STYLE3%> class="INPUT_STYLE3" 
 name="register" value="<%=rs.getString("stock_out_checker") %>"></td>
 ...
 <input name="register_time" type="hidden" value="<%="2010-01-10"%>">
  ...
  name="gatherer_name" readonly="readonly" value="<%=rs.getString("stock_out_check_time") %>" />
 </td>
 </tr>
 </table>
 <%
