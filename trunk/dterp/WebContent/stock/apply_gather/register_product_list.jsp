...
var names=['product_describe_ok','product_ID1','product_name1','tuoPan1','count1','weight1','proId','admin1'];
var a = 0;
function addGoodsItem(product_namea, product_IDa, product_describea, amounta, amount_unita, cost_pricea,proId,admin1) {
...
var values=[product_describea, product_IDa,product_name , amounta, amount_unita, cost_pricea,proId,admin1];
...
<%try{
	...
	//String my_sql_search = "select b.*,CUSTOMER_NAME from package_info as b inner join crm_file on crm_file.id = b.package_custom_id where package_stock='' and is_dissolve=0 and is_out_stock=0 "; // modify by wangshaolin
...
if(request.getParameter("paper")!=null&&request.getParameter("paper").length()!=0){
	y_paper=request.getParameter("paper");
...
<%if(request.getParameter("search")!=null){
	String sql_1 = "select count(id) from product_info where package_id='"+rs1.getString("b.id")+"'";
	ResultSet rs_1 =stock_db2.executeQuery(sql_1);
	String count = "";
	if(rs_1.next()){
		count=rs_1.getString(1);
	}	
%>
['<%=rs1.getString("b.product_spec")%>',
'<%=rs1.getString("b.mold_spec")%>',
'<%=rs1.getString("b.package_pallet")%>',
'<div style="text-decoration : underline;color:#3366FF;CURSOR: hand;" onclick=addGoodsItem("<%=exchange.unHtmls(exchange.toHtml(rs1.getString("b.product_spec")))%>","<%=rs1.getString("b.mold_spec")%>","<%=rs1.getString("b.package_pallet")%>","1","<%=count%>","","<%=rs1.getString("b.id")%>","<%=rs1.getString("CUSTOMER_NAME")%>")><%=demo.getLang("erp","入库")%></div>'
],<%}%>
</page:pages>
['']];
nseer_grid.init();
