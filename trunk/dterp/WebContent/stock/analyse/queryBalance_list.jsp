...
//	现场托盘号
 ...
 <form id="search_form" name="search_form" action="queryBalance_list.jsp" method="post" >
 ...
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="7%"><%=demo.getLang("erp","仓库")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="11%"><select name="product_stock" id="product_stock"  style="width:120px">
 <option value="">--------全部--------</option>
 <%
 	String sql="select * from stock_config_public_char where describe1='库房'";
    ResultSet rs2=stock_db2.executeQuery(sql);
    while(rs2.next()){
 %>
    		<option value="<%=rs2.getString("id") %>"><%=rs2.getString("stock_name") %></option>
   <%
    } 
   %>
 		 </select></td>
 ...
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="7%">&nbsp;</td>		 
 <td class="TD_STYLE8" width="10%" align="right"><%=demo.getLang("erp","状态")%>：</td>
 <td width="7%" align="left"><select name="state" id="state"  style="width:120px">
 			<option value="">--------全部--------</option>
    		...
    		<option value="6">废弃</option>
    		<option value="7">出库</option>
    	
 		 </select></td>

 <td <%=TD_STYLE1%> class="TD_STYLE8" width="7%"></td>
 <td style="text-align:right;" width="15%"><input type="submit" align=right class="BUTTON_STYLE1" value="<%=demo.getLang("erp","查询")%>"></td>
 </tr>
 </table>
 <form>
...
String my_sql_search="SELECT a . * , b.stock_in_time AS stock_in_time, c.stock_out_time AS stock_out_time FROM product_info AS a LEFT JOIN ("+
	...
	"WHERE a.package_id=0 and product_status!=0 and product_status!=8 and product_status!=4 and  ( product_status!=6 or (stock_id = 109 and product_status = 6)) and product_is_production=1 "+
	my_condition;
...
nseer_grid.columns =[
					   ...
					   {name: '<%=demo.getLang("erp","产品状态")%>'},
					   {name: '<%=demo.getLang("erp","库位")%>'},
					   {name: '<%=demo.getLang("erp","临时托盘号")%>'},
					   {name: '<%=demo.getLang("erp","现场托盘号")%>'}
					  
]
nseer_grid.column_width=[120,120,120,120,120,120,120,110,120,120,120];
...
'<%=product_type_str%>',
'<%=product_pstatus_str%>',
'<%=product_pstatus2!=7?rs.getString("product_pstatus"):""%>',
...
init_select("product_type","<%=product_type%>");
init_select("state","<%=product_status%>");
init_select("product_stock","<%=product_stock%>");
</script>
<%
stock_db.close();	
stock_db2.close();
