...
<%if(rs1.getString("paying_or_gathering").equals("入库")){
	balance_amount+=rs1.getDouble("gathered_amount");
		%>
'<%=rs1.getString("gather_ID")%>','','<%=exchange.toHtml(rs1.getString("product_name"))%>','<%=rs1.getString("product_ID")%>','<%=exchange.toHtml(rs1.getString("register_time"))%>','<%=rs1.getDouble("gathered_amount")%>','','<%=balance_amount%>'


			 <%}else{
	balance_amount=balance_amount-rs1.getDouble("paid_amount");
				 %>

	'','<%=rs1.getString("pay_ID")%>','<%=exchange.toHtml(rs1.getString("product_name"))%>','<%=rs1.getString("product_ID")%>','<%=exchange.toHtml(rs1.getString("register_time"))%>','','<%=rs1.getDouble("paid_amount")%>','<%=balance_amount%>'
		<%}%>
		],
</page:pages>
['']];
