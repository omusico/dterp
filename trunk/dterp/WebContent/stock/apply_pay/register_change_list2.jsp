...
var names=['product_name1','product_ID1','product_describe_ok1','amount1','amount_unit1','cost_price1','cost_person1','cost_dopet1','cost_personId','proId'];
...
String fileName="register_change_list2.jsp";
...
'<%=exchange.toHtml(rs1.getString("package_weight"))%>',
'<%=exchange.toHtml(rs1.getString("CUSTOMER_NAME"))%>', 
'<%=rs1.getString("package_info.package_stock")%>',
'<div style="text-decoration : underline;color:#3366FF;CURSOR: hand;" onclick=addGoodsItem("<%=exchange.unHtmls(rs1.getString("product_spec"))%>","<%=exchange.unHtmls(rs1.getString("mold_spec"))%>","<%=rs1.getString("package_pallet")%>","1","<%=rs1.getString("package_count")%>","<%=rs1.getString("package_weight")%>","<%=rs1.getString("CUSTOMER_NAME")%>","<%=rs1.getString("package_info.package_stock")%>","<%=rs1.getString("package_custom_id")%>","<%=rs1.getString("package_info.id")%>")><%=demo.getLang("erp","出库")%></div>'
],<%}%>
</page:pages>
['']];
nseer_grid.init();
