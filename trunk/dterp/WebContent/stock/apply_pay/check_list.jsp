...
String my_sql_search="select * from stock_out_apply where apply_out_check_status=0 ";
...
<table width="100%" style="text-align: right;" align="center">
  <tr>
  	<td>
    	申请单号：<input type="text" name="txtPager"  value="<%=txtPager %>"/>
    </td>
     <td>
     ...
     <td>
    	登记时间：<input type="text" name="txtTime" onkeydown="javascript:this.value='';" onkeyup="javascript:this.value='';" id="date_start" value="<%=txtTime %>"/>
    </td>
    <td>
...
nseer_grid.column_width=[130,130,150,130,130,130,370];
nseer_grid.data = [
...
'<div style="text-decoration : underline;color:#3366FF" onclick="onAuditing(<%=rs1.getString("id")%>,<%=rs1.getString("apply_out_reason_id")%>)">审核</div>'
],<%}%>
</page:pages>
['']];
nseer_grid.init();
