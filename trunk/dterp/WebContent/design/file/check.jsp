...
var res=confirm("是否确认该操作？");
	if(res){
		form.action = "../../design_file_check_delete_ok?id=<%=id%>";
	}else{
		return false;
	}
}else{
	var res=confirm("是否确认该操作？");
	if(res){
		form.action = "../../design_file_check_ok?id=<%=id%>";
	}else{
		return false;
	}
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		...
		<div <%=DIV_STYLE1%> class="DIV_STYLE1"><INPUT name="Ref"
			type="radio" <%=RADIO_STYLE1%> class="RADIO_STYLE1" value=cop checked onclick="changeRadio(this)"><%=demo.getLang("erp", "未通过")%>
		<INPUT name="Ref" type="radio" <%=RADIO_STYLE1%> class="RADIO_STYLE1"
			value=Ind onclick="changeRadio(this)"> <%=demo.getLang("erp", "通过")%> <input
			type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1"
			value="<%=demo.getLang("erp","确认")%>" name="B1">&nbsp;<input
...
<table <%=TABLE_STYLE1%> class="TABLE_STYLE1" id=theObjTable>
	<tr style="background-image:url(../../images/line.gif)">
		<td colspan="4">
		<div style="width:100%; height:12; padding:3px; "><%=demo.getLang("erp", "产品信息审核")%></div>
		</td>
	</tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
	...
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		...
		<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input
			<%=INPUT_STYLE1%> class="INPUT_STYLE1" name="checker" type="text"
			style="width:49%" value="<%=checker %>" onFocus="this.blur()">

		</td>
		<td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp", "审核时间")%></td>
		<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input
			<%=INPUT_STYLE1%> class="INPUT_STYLE1" name="check_time" type="text"
			style="width:49%" value="<%=time.substring(0,10) %>" onFocus="this.blur()"></td>
	</tr>
