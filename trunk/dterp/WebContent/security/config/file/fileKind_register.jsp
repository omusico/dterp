...
<%
		...
		if (tag.equals("0")) {
			...
			String stock_ID = count.read((String) session
			.getAttribute("unit_db_name"), "stockstockcount")
			+ "";
			stock_ID = stock_ID.substring(1, 3);
			int filenum = count.read((String) session
			.getAttribute("unit_db_name"), "stockstockcount");
			count.write((String) session.getAttribute("unit_db_name"),
			"stockstockcount", filenum);
			...
			String sql = "delete from stock_config_public_char where stock_name='"
			+ file_name + "库房' and describe1='库房'";
			hr_db.executeUpdate(sql);
		}
		hr_db.close();
	} catch (Exception ex) {
