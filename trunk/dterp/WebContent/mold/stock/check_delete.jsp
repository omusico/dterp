...
<% 
	...
	try {
		String sql = "DELETE FROM mold_stock WHERE mold_id = '" + id
		+ "'";
		mold_db.executeUpdate(sql);
		sql = "DELETE FROM mold_stock_pic WHERE mold_id = '" + id + "'";
		mold_db.executeUpdate(sql);
		sql = "update mold_info set mold_life_status ='',stock_time='' WHERE id = '"
		+ id + "'";
		mold_db.executeUpdate(sql);
		mold_db.close();
	} catch (Exception ex) {
...
<table class="TABLE_STYLE2" border="0">
	<tr class="TR_STYLE1">
		<td>
		<div>&nbsp;&nbsp;该记录未通过审核,请返回!</div>
		</td>
		<td class="TD_STYLE3">
		<div class="DIV_STYLE1"><input type="button" align="right"
			class="BUTTON_STYLE1" value="返回" onClick=location="check_list.jsp"></div>
		</td>
	</tr>
</table>
