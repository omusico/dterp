...
String assembler="";
String my_sql_search="SELECT * FROM mold_info a,mold_assembly_installation b where a.mold_life_status=3 and a.id=b.mold_id";
...
nseer_grid.column_width=[200,200,100,150,150,150];
...
--%>
['<%=rs1.getString("mold_spec")%>','<%=rs1.getString("mold_code")%>','<%=rs1.getString("assembler")%>','<%=rs1.getString("assembly_time")%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?id=<%=rs1.getString("id")%>")><%=demo.getLang("erp","审核")%></div>'],
<%--
