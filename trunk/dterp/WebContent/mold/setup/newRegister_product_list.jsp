...
<table <%=TABLE_STYLE3%> class="TABLE_STYLE3">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		...
		<div class="div_handbook"><%="您正在做的业务是：模具管理--模具组装管理--模具组装登记--模具添加"%></div>
		</td>
	</tr>
</table>
...
int i = 0;
	...
	"where mold_info.mold_life_status='4'";
...
nseer_grid.column_width=[265,265,120];
...
String sql="select * from crm_file where id='"+rs1.getString("mold_purchase_supplier_id")+"'";
 ...
 if(rs1.getString("stock_time")!=null&&rs1.getString("stock_time").length()>9)
 {
	stock_time=rs1.getString("stock_time").substring(0,10);	 
 }
	
 %>
['<%=rs1.getString("mold_spec") %>','<%=rs1.getString("mold_code") %>','<div style="text-decoration : underline;color:#3366FF;CURSOR: hand;" onClick=addSpec("<%=exchange.unHtmls(rs1.getString("mold_spec")) %>","<%=exchange.unHtmls(rs1.getString("mold_code")) %>","<%=exchange.unHtmls(Integer.toString(rs1.getInt("mold_info.id")))%>","<%=exchange.unHtmls(stock_time) %>","<%=CUSTOMER_NAME%>","<%=exchange.unHtmls(rs1.getString("purchase_code"))%>","<%=rs1.getString("assembler")%>","<%=rs1.getString("assembly_time")%>","<%=rs1.getString("top_mold_code")%>","<%=rs1.getString("bottom_mold_code")%>","<%=rs1.getString("lock_code")%>")><%=demo.getLang("erp","选择")%></div>'],	
...
function addSpec(strSpec,strLot_no,id,stock_time,mold_purchase_supplier_id,mold_purchase_id,assembly,assembly_time,top1,bottom2,lock1)
{
//alert(stock_time);
//alert("1111");
 ...
 var mold_purchase_supplier_ida=mold_purchase_supplier_id.replace(/★/g,"<br>").replace(/☆/g," ");
 var mold_purchase_ida=mold_purchase_id.replace(/★/g,"<br>").replace(/☆/g," ");
 var stock_timea=stock_time.replace(/★/g,"<br>").replace(/☆/g," ");
 var assemblya=assembly.replace(/★/g,"<br>").replace(/☆/g," ");
 var assembly_timea=assembly_time.replace(/★/g,"<br>").replace(/☆/g," ");
 //
 var top1a=top1.replace(/★/g,"<br>").replace(/☆/g," ");
 var bottom2a=bottom2.replace(/★/g,"<br>").replace(/☆/g," ");
 var lock1a=lock1.replace(/★/g,"<br>").replace(/☆/g," ");

 window.opener.document.getElementById("mold_purchase_supplier").value =mold_purchase_supplier_ida;
 window.opener.document.getElementById("purchase_code").value =mold_purchase_ida;
 ...
 window.opener.document.getElementById("stock_time").value =stock_timea;
 window.opener.document.getElementById("assembler").value =assemblya;
 window.opener.document.getElementById("assembly_time").value =assembly_timea;
 
 window.opener.document.getElementById("top_mold_code").value =top1a;
 window.opener.document.getElementById("bottom_mold_code").value =bottom2a;
 window.opener.document.getElementById("lock_code").value =lock1a;
