...
String mold_spec="";
String mold_code="";
String my_sql_search="SELECT * FROM mold_exception left join mold_info on mold_exception.mold_id=mold_info.id where mold_status!=1  ";
...
<table width="100%" border="0" style=" text-align: right;">
  <tr>
    ...
    审核状态：
		  ...
		  <%
		  if (request.getParameter("plan_typeS") != null&&request.getParameter("plan_typeS").length() > 0) {
				if(request.getParameter("plan_typeS").equals("2")){
		  %>
...
nseer_grid.column_width=[200,100,100,150,150,100,100];
...
--%><%if(request.getParameter("mold_spec")!=null){%>
	...
	'<%=rs1.getString("exception_regist_time")%>','<%=status%>','<div style="text-decoration : underline;color:#3366FF;" onclick=id_link("change_list_look.jsp?id=<%=rs1.getString("mold_exception.id")%>&mold_id=<%=rs1.getString("mold_id")%>")>变更</div>'],
<%--
