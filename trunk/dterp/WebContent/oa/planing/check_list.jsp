...
nseer_grid.column_width=[160,200,140,140,
<%
for(int i=1;i<workflow_amount;i++){
%>					   70,
<%
...
%>	['<%=exchange.toHtml(rs.getString("subject"))%>','<%=exchange.toHtml(rs.getString("type"))%>','<%=exchange.toHtml(rs.getString("begin_time"))%>','<%=exchange.toHtml(rs.getString("end_time"))%>'
<%
for(int i=1;i<=workflow_amount;i++){
	String status="";
	if(rs2.next()){
