...
function delInfo(){
	...
	if(res){
		mutiValidation.action="../../crm_file_del_ok";
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 ...
 <%
 		...
 		//客户类型
 		String type_name = "";
 		String sql_type = "select * from crm_config_public_char where kind='客户类型' and id="
 				+ rss.getString("type") + " order by type_ID";
...
<table <%=TABLE_STYLE1%> class="TABLE_STYLE1" id=theObjTable>
	<tr style="background-image:url(../../images/line.gif)">
		<td colspan="4">
		<div style="width:100%; height:12; padding:3px; "><%=demo.getLang("erp", "客户档案变更")%></div>
		</td>
	</tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		...
		<td <%=TD_STYLE21%> class="TD_STYLE2" width="39%"><input
			id="validator_dup" type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5" style="width:80%"
			name="customer_name" value="<%=rss.getString("customer_name") %>" onblur="userCheck();"><a
		...
		<td <%=TD_STYLE21%> class="TD_STYLE2" width="39%"><input
			...
			name="customer_address" style="width:80%" value="<%=rss.getString("customer_address") %>"></td>
	</tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		<td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><font color=red>*</font><%=demo.getLang("erp", "客户分类")%></td>
		<td <%=TD_STYLE21%> class="TD_STYLE2" width="39%">
		<input type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5" id="fileKind_chain" name="fileKind_chain" style="width:80%" onFocus="showKind('nseer1',this,showTree)" onkeyup="search_suggest(this.id)" value="<%=rss.getString("chain_id")%> <%=exchange.toHtml(rss.getString("chain_name"))%>">
		...
		<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><select
			...
			<%
					...
					while (rs4.next()) {
						if(rss.getString("type").equals(rs4.getString("id")))
						{
			%>
			<option value="<%=exchange.toHtml(Integer.toString((rs4.getInt("id"))))%>" selected="selected"><%=rs4.getString("type_name")%></option>
			
			<%				
						}else{
			%>
			<option value="<%=exchange.toHtml(Integer.toString((rs4.getInt("id"))))%>"><%=rs4.getString("type_name")%></option>
			<%
						}
			}
			%>
	...
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		...
		<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input
			...
			style="width:45%" value="<%=rss.getString("customer_sf") %>"></td>
	...
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		...
		<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input
			type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5"
			name="contact_person1" style="width:45%" value="<%=rss.getString("contact_person1") %>"></td>
		<td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp", "联系人2")%></td>
		<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input
			type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5"
			name="contact_person2" style="width:45%" value="<%=rss.getString("contact_person2") %>"></td>
	</tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		<td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp", "电话1")%></td>
		<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input
			type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5"
			name="customer_tel1" style="width:45%" value="<%=rss.getString("CUSTOMER_TEL1") %>"></td>
		<td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp", "电话2")%></td>
		<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input
			type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5"
			name="customer_tel2" style="width:45%" value="<%=rss.getString("CUSTOMER_TEL2") %>"></td>
	</tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		<td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp", "传真")%></td>
		<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input
			type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5"
			name="customer_fax" style="width:45%" value="<%=rss.getString("CUSTOMER_FAX") %>">&nbsp;例如：XXXX-XXXXXXXX</td>
		<td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp", "邮政编码")%></td>
		<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input
			type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5"
			name="customer_postcode" style="width:45%" value="<%=rss.getString("CUSTOMER_POSTCODE") %>"></td>
	</tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		<td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp", "登记人")%></td>
		<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%">
		<input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="register" style="width:45%" value="<%=exchange.toHtml(operator)%>" onFocus="this.blur()"></td>
		<td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp", "登记时间")%></td>
		<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%">
		<input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="register_time" style="width:45%" value="<%=exchange.toHtml(time.substring(0,10))%>" onFocus="this.blur()"></td>
	</tr>
	<%
