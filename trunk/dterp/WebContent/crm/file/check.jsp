...
if (form.Ref[0].checked){
	...
	if(res){
		form.action = "../../crm_file_check_delete_ok?config_id=<%=config_id%>&customer_ID=<%=customer_ID%>";
	}else{
		return;
	}
}else{
...
<form id="mutiValidation" class="x-form" method="post"
	...
	<input type="hidden" name="id" style="width:49%" value="<%=id%>">
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<table <%=TABLE_STYLE1%> class="TABLE_STYLE1" id=theObjTable>
	<tr style="background-image:url(../../images/line.gif)">
		<td colspan="4">
		<div style="width:100%; height:12; padding:3px;"><%=demo.getLang("erp", "客户信息审核")%></div>
		</td>
	</tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
	...
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		...
		<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="checker" type="text" value="<%=checker %>" style="width: 45%" onFocus="this.blur()"></td>
		<td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp", "审核时间")%></td>
		<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="check_time" type="text" value="<%=time.substring(0,10) %>" style="width: 45%" onFocus="this.blur()"></td>
	</tr>
