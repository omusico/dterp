...
<form id="mutiValidation" method="POST"
	action="../../manufacture_apply_register_ok"
	onSubmit="return doValidate('../../xml/manufacture/manufacture_apply.xml','mutiValidation')">
...
<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		<td <%=TD_STYLE1%> class="TD_STYLE8"><input type="submit"
...
<%
		...
		String sql = "select distinct product_ID from stock_pay_details where "
		+ condition + "";
		ResultSet rs = stockdb.executeQuery(sql);
		while (rs.next()) {
			double amount1 = 0.0d;
			String sql1 = "select sum(apply_manufacture_amount) from stock_pay_details where product_ID='"
			...
			if (rs1.next()) {
		amount1 = rs1
				.getDouble("sum(apply_manufacture_amount)");
			}
		...
		for (i = 0; i < m; i++) {
			String sql2 = "select distinct pay_ID,product_ID,product_name,product_describe,type from stock_pay_details where product_ID='"
			+ product_ID1[i] + "'";
		...
		String sql3 = "select sum(apply_manufacture_amount) from stock_pay_details where product_ID='"
				+ product_ID1[i]
				+ "' and pay_ID='"
				+ rs2.getString("pay_ID") + "'";
		ResultSet rs3 = stock_db.executeQuery(sql3);
		if (rs3.next()) {
			amount2 = rs3
			.getDouble("sum(apply_manufacture_amount)");
		}
		int a = choice_group.indexOf(", "
				+ rs2.getString("pay_ID") + ", ");
		if (a != -1) {
			if (pay_ID_group[i] == null) {
				pay_ID_group[i] = rs2.getString("pay_ID");
			} else {
				pay_ID_group[i] = pay_ID_group[i] + ", "
				+ rs2.getString("pay_ID");
			}
		}
			}
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4" id=theObjTable>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		...
		<td <%=TD_STYLE2%> class="TD_STYLE2" width="89%"><input
			<%=INPUT_STYLE3%> class="INPUT_STYLE3" name="designer" type="text"
			style="width:44.8%"></td>
	</tr>
</table>
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
...
<TABLE <%=TABLE_STYLE5%> class="TABLE_STYLE5">
	<tr <%=TR_STYLE2%> class="TR_STYLE2">
		...
		<td <%=TD_STYLE2%> class="TD_STYLE2" width="11%"><%=demo.getLang("erp", "描述")%>
		</td>
	...
	%>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		<td <%=TD_STYLE2%> class="TD_STYLE2"><input type="hidden"
			name="product_ID" value="<%=product_ID[j]%>"><%=product_ID[j]%>&nbsp;</td>
		<td <%=TD_STYLE2%> class="TD_STYLE2"><input type="hidden"
			name="product_name" value="<%=exchange.toHtml(product_name[j])%>"><%=exchange.toHtml(product_name[j])%>&nbsp;</td>
		<td <%=TD_STYLE2%> class="TD_STYLE2"><input type="hidden"
			name="product_describe" value="<%=product_describe[j]%>"><%=product_describe[j]%>&nbsp;</td>
		<td <%=TD_STYLE2%> class="TD_STYLE2"><input type="hidden"
			name="type" value="<%=exchange.toHtml(type[j])%>"><%=exchange.toHtml(type[j])%>&nbsp;</td>
		<td <%=TD_STYLE2%> class="TD_STYLE2"><input type="hidden"
			name="amount" value="<%=amount[j]%>"><%=new java.text.DecimalFormat((String) application
									.getAttribute("nseerAmountPrecision"))
									.format(amount[j])%>&nbsp;</td>
		<td <%=TD_STYLE2%> class="TD_STYLE2" colspan="3"><input
			type="hidden" name="pay_ID_group" value="<%=pay_ID_group[j]%>"><%=pay_ID_group[j]%>&nbsp;</td>
	</tr>
	<%
	}
	%>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		...
		<td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input
			name="register_time" type="hidden" style="width: 50%; "
			value="<%=exchange.toHtml(time)%>"><%=exchange.toHtml(time)%>&nbsp;</td>
	</tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		<td <%=TD_STYLE1%> class="TD_STYLE8" width="10%"><%=demo.getLang("erp", "备注")%>&nbsp;&nbsp;&nbsp;&nbsp;：</td>
		<td <%=TD_STYLE2%> class="TD_STYLE2" width="89%" colspan="3"><textarea
			<%=TEXTAREA_STYLE1%> class="TEXTAREA_STYLE1" name="remark" cols="115"
			rows="4"></textarea></td>
	</tr>
</table>
