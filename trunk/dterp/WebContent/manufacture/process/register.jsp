...
<%
		...
		String my_condition=" and C_DEFINE1=0 ";
		//计划类型查询
		...
		//查询条件加入check_tag不等于2,标示未删除
		String my_sql_search = "select id,plan_id,plan_type,plan_maker,plan_register,plan_register_time,plan_check_tag FROM product_plan where (plan_check_tag=1 or plan_check_tag=3) "+ my_condition + " order by id desc ";
...
<form action="register.jsp" method="post" name="search_form" id="search_form">
<table <%=TABLE_STYLE3%> class="TABLE_STYLE3">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		<td <%=TD_STYLE6%> class="TD_STYLE6">
		登记时间：
		<input type="text" name="register_timeS" onfocus="" id="date_start" value="<%=register_timeS %>" style="width: 150">
		&nbsp;
		生产计划类型：
...
nseer_grid.columns =[
	                   ...
	                   {name: '<%=demo.getLang("erp","进入")%>'}
]
nseer_grid.column_width=[200,200,200,200,200,160];
...
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("register_2.jsp?id=<%=rs.getString("id") %>")><%=demo.getLang("erp", "进入")%></div>'],
