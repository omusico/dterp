...
public class unperiodFinish_ok extends HttpServlet{
				...
				if(!rs.next()){
					...
					if(!rs.next()){
						...
						if(!rs.next()){
							...
							rs=finance_db.executeQuery(sql);
							if(!rs.next()){
								sql="update finance_account_period set account_finished_tag='0',account_finished_time='"+account_finished_time+"' where account_period='"+account_period+"'";
								finance_db.executeUpdate(sql);
								response.sendRedirect("finance/account/unperiodFinish_ok_a.jsp");
							}else{
								response.sendRedirect("finance/account/unperiodFinish.jsp");
							}
						}else{
							response.sendRedirect("finance/account/unperiodFinish.jsp");
						}
