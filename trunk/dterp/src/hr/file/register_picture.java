		...
		try {
			...
			if (hr_db.conn((String) dbSession.getAttribute("unit_db_name"))) {
				...
				if (!chain_id.equals("")) {
					...
					String human_title_class = request.getParameter("select5");
					human_title_class=human_title_class.substring(3, human_title_class.length());
					...
					*/
					...
					try {
						...
						if (rset.next()) {
							...
							response.sendRedirect("security/file/register_choose_picture.jsp?human_ID="
											+ human_ID + "");
					...
					response.sendRedirect("security/file/register_picture_b.jsp");
