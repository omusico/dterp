...
public class change_key_ok extends HttpServlet{
...
stock_db.executeUpdate(sql5) ;
	...
	String sql4 = "delete from stock_cell_details where design_ID='"+design_ID+"' and stock_name='"+stock_name+"'" ;
...
response.sendRedirect("stock/cell/change_key_ok_a.jsp");
}else{
response.sendRedirect("stock/cell/change_key_ok_b.jsp?design_ID="+design_ID+"");
}
}else{
response.sendRedirect("stock/cell/change_key_ok_c.jsp");
}
stock_db.commit();
stock_db.close();
}else{
