...
design_ID=main_code+first_code+second_code+design_ID;
...
response.sendRedirect("stock/cell/register_key_ok_a.jsp");
}else{
	session.setAttribute("product_name",product_name);
response.sendRedirect("stock/cell/register_key_ok_b.jsp?product_ID="+product_ID+"");
}
}else{
response.sendRedirect("stock/cell/register_key_ok_c.jsp");
}
stock_db.commit();
stock_db.close();
}else{
