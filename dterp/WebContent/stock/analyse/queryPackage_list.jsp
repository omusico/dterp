...
//	库位状态
	...
	if(lotno!=null&&!lotno.trim().equals("")){
		my_condition +=" and id in (select package_id from product_info where product_lot_no like '%"+lotno+"%' )";
	}else{
		lotno="";
	}
	 %>
 ...
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp","原纸规格")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="13%"><input name="product_spec"  type="text"  value="<%=product_spec %>" Style="width=120px" ></td>
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp","模具规格")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="13%"><input name="mold_spec"  type="text"  value="<%=mold_spec %>" Style="width=120px"></td>

 
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp","栈板号")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="13%"><input name="package_pallet"  type="text"  value="<%=package_pallet %>" Style="width=120px"></td>
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp","Lot No")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="13%"><input name="lotno"  type="text"  value="<%=lotno %>" Style="width=120px"></td>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="13%"><input name="package_factory_pallet"  type="text"  value="<%=package_factory_pallet %>" Style="width=120px"></td>
 <td class="TD_STYLE8" width="9%">库位：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="13%"><input name="package_stock"  type="text"  value="<%=package_stock %>" Style="width=120px"></td>
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="10%"><%=demo.getLang("erp","是否解散")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="13%"><select name="is_dissolve" id="is_dissolve"  style="width:120px">
 			<option value="">--------全部--------</option>
    		<option value="1">是</option>
    		<option value="0">否</option>
     </select></td>
 <td align=center class="TD_STYLE8" width="9%">&nbsp;</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="13%">&nbsp;</td>
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="9%">&nbsp;</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="13%">&nbsp;</td>
  <td align=right class="TD_STYLE2" width="19%"><input type="submit" align=right class="BUTTON_STYLE1" value="<%=demo.getLang("erp","查询")%>"></td>
 </tr>			
...
String my_sql_search = "select * FROM package_info where is_out_stock!=3  "+my_condition;
...
nseer_grid.columns =[
					   ...
					   {name: '<%=demo.getLang("erp","重量")%>'},
					   {name: '<%=demo.getLang("erp","库位")%>'},
					   {name: '<%=demo.getLang("erp","客户")%>'},
					   {name: '<%=demo.getLang("erp","是否被解散")%>'},
					   {name: '<%=demo.getLang("erp","是否出库")%>'},
					    {name: '<%=demo.getLang("erp","查看")%>'}
                       
					  
]
nseer_grid.column_width=[120,120,90,90,100,100,100,100,100,80,80];
...
%>
[
<%if(is_dissolve_str.equals("是")){%>
'<span style="color:#FF3333;"><%=rs.getString("product_spec")%></span>',
'<span style="color:#FF3333;"><%=rs.getString("mold_spec")%></span>',
'<span style="color:#FF3333;"><%=rs.getString("package_pallet")%></span>',
'<span style="color:#FF3333;"><%=rs.getString("package_factory_pallet")%></span>',
'<span style="color:#FF3333;"><%=rs.getString("package_count")%></span>',
'<span style="color:#FF3333;"><%=rs.getString("package_weight")%></span>',
'<span style="color:#FF3333;"><%=rs.getString("package_stock")%></span>',

'<span style="color:#FF3333;"><%=package_custom%></span>',
'<span style="color:#FF3333;"><%=is_dissolve_str%></span>',
'<span style="color:#FF3333;"><%=is_out_stock_str%></span>','<div style="text-decoration : underline;color:#FF3333";font-size:14px; onclick=id_link("read_queryPackage_list.jsp?id=<%=rs.getString("id")%>")><span>查看</span></div>'
<%}else{%>
'<%=rs.getString("product_spec")%>',
'<%=rs.getString("mold_spec")%>',
'<%=rs.getString("package_pallet")%>',
'<%=rs.getString("package_factory_pallet")%>',
'<%=rs.getString("package_count")%>',
'<%=rs.getString("package_weight")%>',
'<%=rs.getString("package_stock")%>',

'<%=package_custom%>',
'<%=is_dissolve_str%>',
'<%=is_out_stock_str%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("read_queryPackage_list.jsp?id=<%=rs.getString("id")%>")><span>查看</span></div>'
<%}%>
],
<%k++;%>
</page:pages>  
...
init_select("is_dissolve","<%=is_dissolve%>");
