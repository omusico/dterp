...
<script type="text/javascript" src="../../javascript/include/validate/validation-framework.js"></script>
 ...
 //HttpSession dbSession = request.getSession();// 获取会话session
	...
	String sql = "select * from stock_in inner join hr_file on hr_file.idcard=stock_in.stock_in_operator_id where stock_in.id = "+id;
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%" style="text-align: left"><input type="text" readonly="readonly" style="border-bottom: 1px solid #000;border-top: 0px solid #000;border-left: 0px solid #000;border-right:0px solid #000; " name="gatherer_name" value="<%=rs.getString("stock_in.stock_in_code")  %>"></td>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%" style="text-align: left"><input type="text" readonly="readonly" style="border-bottom: 1px solid #000;border-top: 0px solid #000;border-left: 0px solid #000;border-right:0px solid #000; " name="gatherer_name" value="<%=request.getParameter("audOrder") %>"></td>
  ...
  <tr>
  <td <%=TD_STYLE1%> class="TD_STYLE8" width="11%">入库人：</td><td  width="40%"><input type="text" readonly="readonly" style="border-bottom: 1px solid #000;border-top: 0px solid #000;border-left: 0px solid #000;border-right:0px solid #000; "name="gatherer_name" value="<%=rs.getString("hr_file.human_name") %>"></td>
...
<td <%=TD_STYLE2%> class="TD_STYLE2" width="10%"><%=demo.getLang("erp","入库状态")%></td>
 ...
 <%
	sql = "select * from stock_in_detail inner join product_info on product_info.id =stock_in_detail.In_Detail_product_id   where stock_in_id='"+rs.getString("id")+"'";
...
<table <%=TABLE_STYLE5%> class="TABLE_STYLE5" cols=1 id=tableOnlineEdit1
	...
	<thead>
		...
		<%
					String sql_t = "select * from stock_in_detail inner join package_info on package_info.id =stock_in_detail.In_Detail_product_id  where stock_in_id='"+ rs.getString("id") + "'";
...
<textarea <%=TEXTAREA_STYLE1%> class="TEXTAREA_STYLE1" name="remark" readonly="readonly"><%=rs.getString("stock_in_remark") %></textarea>
</td>
 </tr>--%>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
