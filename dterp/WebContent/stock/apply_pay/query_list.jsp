...
String mod=request.getRequestURI();
 ...
 <script>
 	function openSearch(ag,productId){
 		window.location.href="search_detail.jsp?order_id="+ag+"&proId="+productId;
 	}
 </script>
<%
...
String my_sql_search = "select * from stock_out_apply where 1=1 ";
...
if(rs1.getString("apply_out_check_status").equals("0")){
	sh = "未审核";
}else if(rs1.getString("apply_out_check_status").equals("1")){
	sh = "审核通过";
}
...
'<%=sh%>',
'<%=rk%>',
'<div style="text-decoration : underline;color:#3366FF;" onclick="openSearch(<%=rs1.getString("id")%>,<%=rs1.getString("apply_out_reason_id")%>)">查看</div>'
],<%}%>
</page:pages>
['']];
nseer_grid.init();
