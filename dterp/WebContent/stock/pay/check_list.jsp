...
String my_sql_search="select * from stock_out inner join hr_file on hr_file.idcard=stock_out.stock_out_operator_id where stock_out_check_status=0";
if(request.getParameter("txtPager")!=null&&request.getParameter("txtPager").length()!=0){
	txtPager=request.getParameter("txtPager");
	my_sql_search+=" and stock_out_id like '%"+request.getParameter("txtPager")+"%'";
	...
	my_sql_search+=" and stock_out_time like '%"+request.getParameter("txtLoginPerson")+"%'";
}
...
<script>
	...
	function onAuditing(ag,productId){
		window.location.href="check_list_auditing.jsp?order_id="+ag+"&proId="+productId;
	}
</script>
...
nseer_grid.columns =[
					   ...
					   {name: '<%=demo.getLang("erp","出库人")%>'},
                       {name: '<%=demo.getLang("erp","出库时间")%>'},
                       {name: '<%=demo.getLang("erp","总件数")%>'},
                       {name: '<%=demo.getLang("erp","出库状态")%>'},
                       {name: '<%=demo.getLang("erp","审核")%>'}
]
nseer_grid.column_width=[160,160,160,150,160,90,290];
nseer_grid.data = [
<page:pages rs="<%=rs1%>" strPage="<%=strPage%>"> 
<%
String sql_1= "SELECT STOCK_NAME FROM stock_config_public_char where id="+rs1.getInt("stock_out_reason_id");
String id ="";
...
%>
['<%=rs1.getString("stock_out_id")%>','<%=id%>','<%=rs1.getString("hr_file.human_name")%>',
'<%=exchange.toHtml(outTime)%>','<%=exchange.toHtml(rs1.getString("stock_out_count"))%>','完成',
'<div style="text-decoration : underline;color:#3366FF" onclick="onAuditing(<%=rs1.getString("stock_out.id")%>,<%=rs1.getString("stock_out_reason_id")%>)">审核</div>'
],<%}%>
</page:pages>
['']];
nseer_grid.init();
