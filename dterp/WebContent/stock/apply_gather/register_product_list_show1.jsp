...
var names=['product_name','product_ID','product_describe_ok','amount','amount_unit','cost_Weight','product_describeN'];
function addGoodsItem(product_namea, product_IDa, product_describea, amounta, amount_unita, cost_pricea,product_describeN) {
...
var values=[product_name, product_IDa, describe,describe, amounta, amount_unit, cost_pricea,product_describeN];
...
String fileName="register_product_list_show1.jsp";
...
--%>
<form  name="search_form" id="search_form" action="register_product_list_show1.jsp" method="post">
<table width="98%" style="text-align: right;" align="center">
  <tr>
  	 <td>
  	 原纸规格：<input type="text" size="40" name="txtPager" value="<%=product_no %>"  /><input type="hidden" value="40" name="searchId"  />&nbsp;<input type="submit" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","查询")%>">
    </td>
  </tr>
</table>
</form>
...
'<%=rs1.getString("product_Name")%>',
	...
	'<div style="text-decoration : underline;color:#3366FF;CURSOR: hand;" onclick=addGoodsItem("<%=exchange.unHtmls(exchange.toHtml(rs1.getString("product_name")))%>","","","","","","")><%=demo.getLang("erp","入库")%></div>'
