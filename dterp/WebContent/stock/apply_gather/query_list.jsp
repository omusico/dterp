...
String mod=request.getRequestURI();
 ...
 <script>
 	function openSearch(ag,av){
 		window.location.href="search_detail.jsp?id="+ag+"&proId="+av;
 	}
 </script>
<%
...
String my_sql_search = "select * from stock_in_apply where 1=1 ";
if(request.getParameter("txtPager")!=null&&request.getParameter("txtPager").length()!=0){
	...
	if(request.getParameter("plan_typeS").equals("all")){
		...
		my_sql_search+=" and apply_in_apply_check_status = '"+request.getParameter("plan_typeS")+"'";
	}
}
...
<table width="100%" style="text-align: right;" align="center">
  <tr>
  	<td>
    	入库申请单编号：<input type="text" name="txtPager"  size="13" value="<%=txtPager %>" />
...
nseer_grid.column_width=[180,150,130,130,160,80,120,120,100];
...
if(rs1.getString("apply_in_apply_check_status").equals("0")){
	sh = "未审核";
}else if(rs1.getString("apply_in_apply_check_status").equals("1")){
	sh = "审核通过";
}else if(rs1.getString("apply_in_apply_check_status").equals("2")){
	sh = "审核未通过";
}
if(rs1.getString("apply_in_apply_check_status").equals("0")){
	rk = "未执行";
...
'<%=sh%>',
'<%=rk%>',
'<div style="text-decoration : underline;color:#3366FF;" onclick="openSearch(<%=rs1.getString("id")%>,<%=rs1.getString("apply_in_apply_reason_id")%>)">查看</div>'
],<%}%>
</page:pages>
['']];
nseer_grid.init();
