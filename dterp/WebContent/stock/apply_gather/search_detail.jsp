...
//打印条形码
function printNumber(ag,proId){
	window.location.href="search_detail1.jsp?orderId="+ag+"&proId="+proId;	
}
//打印入库单
function delSelect(){
	window.print();
...
<form id="mutiValidation" method="POST" action="register_reconfirm.jsp" onSubmit="return doValidate('../../xml/stock/stock_apply_gather.xml','mutiValidation')">
 ...
 <%-- 
<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" onclick="delSelect()" value="<%=demo.getLang("erp","打印入库单")%>">&nbsp;

<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" onclick="printNumber(<%=id %>,<%=proId %>)" value="<%=demo.getLang("erp","打印条形码")%>">
--%>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1" >
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><input <%=INPUT_STYLE4%> value="<%=rs45.getString("In_Detail_fault_number") %>" class="INPUT_STYLE4" name="product_name" type="text" onFocus="this.blur()"></td>
 <td <%=TD_STYLE2%> class="TD_STYLE2"><input <%=INPUT_STYLE4%> value="<%=rs45.getString("In_Detail_fault") %>" class="INPUT_STYLE4" name="product_name" type="text" onFocus="this.blur()"></td>
 </tr>
 <% }%>
</table>
<%}else if(proId==4){ %>
<table <%=TABLE_STYLE5%> class="TABLE_STYLE5" cols=1 id=tableOnlineEdit1
	...
	<thead>
		...
		<tr <%=TR_STYLE1%> class="TR_STYLE1">
			...
			<td <%=TD_STYLE2%> class="TD_STYLE2"><input <%=INPUT_STYLE4%>
				class="INPUT_STYLE4" name="cost_price" type="text"
				onFocus="this.blur()" value="<%=custem%>">
...
<%String loginT = rs.getString("apply_in_apply_register_time");loginT=loginT.substring(0,10); %>
 ...
 <input type="text" readonly="readonly" style="border-bottom: 1px solid #000;border-top: 0px solid #000;border-left: 0px solid #000;border-right:0px solid #000; " name="gatherer_name"  value="<%=loginT %>" />
</td>
 </tr>
 <%if(!(rs.getString("apply_in_apply_check_status").equals("0"))){ %>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">

 <td <%=TD_STYLE1%> class="TD_STYLE8" width="11%"><%=demo.getLang("erp","审核人")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%" style="text-align: left"><input type="text" readonly="readonly" style="border-bottom: 1px solid #000;border-top: 0px solid #000;border-left: 0px solid #000;border-right:0px solid #000; " name="register" value="<%=rs.getString("apply_in_apply_checker") %>"></td>
 ...
 <input name="register_time" type="hidden" value="<%="2010-01-10"%>">
  <input type="text" readonly="readonly" style="border-bottom: 1px solid #000;border-top: 0px solid #000;border-left: 0px solid #000;border-right:0px solid #000; " name="gatherer_name"  value="<%=rs.getString("apply_in_apply_check_time")%>" />
 </td>
 </tr>
 <%} %>
