...
String moon=time.substring(0, 7);
String moon1=time.substring(5,7);//月
String moon2=time.substring(2,4);//年
moon1=moon2+moon1;
%>
<table <%=TABLE_STYLE3%> class="TABLE_STYLE3">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		<td <%=TD_HANDBOOK_STYLE1%> class="TD_HANDBOOK_STYLE1">
		<div class="div_handbook"><%="您正在做的业务是：模具管理--模具采购管理--模具采购登记"%></div>
		</td>
	</tr>
</table>
...
function addSpec(strSpec,strLot_no,time,a,b)
{
 var  strSpeca=strSpec.replace(/★/g,"<br>").replace(/☆/g," ");
 var  strLot_noa=strLot_no.replace(/★/g,"<br>").replace(/☆/g," ");
  var  timea=time.replace(/★/g,"<br>").replace(/☆/g," ");
 window.opener.document.getElementById("purchase_supplier_id").value =strSpeca;
 window.opener.document.getElementById("ida").value =strLot_noa;
  window.opener.document.getElementById("purchase_id").value =timea+'-'+a+'-'+b;
 this.close();
}

</script>
<%	
String sql = "SELECT count(id) from mold_purchase_order where purchase_register_time like'%"+moon+"%'";
ResultSet rs2 = mold_db.executeQuery(sql);
rs2.next();
int n=Integer.parseInt(rs2.getString("count(id)"));
n=n+1;
...
if(n<10)
{
	...
	String my_sql_search = "SELECT CUSTOMER_NAME,id,customer_sf FROM crm_file where type='8'";
...
--%>
<form action="newRegister_product_list1.jsp" method="post" name="search_form" id="search_form">
<table align=center width="99%" border="0">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		<td <%=TD_STYLE6%> class="TD_STYLE6">		
		  供应商名称<input type="text" name="keyword" onfocus="" id="" value="" style="width: 18%">&nbsp;
...
nseer_grid.column_width=[530,120,];
nseer_grid.auto='<%=demo.getLang("erp","产品名称")%>';
nseer_grid.data = [

<page:pages rs="<%=rs1%>" strPage="<%=strPage%>"> 

['<%=rs1.getString("CUSTOMER_NAME") %>','<div style="text-decoration : underline;color:#3366FF;CURSOR: hand;" onClick=addSpec("<%=exchange.unHtmls(rs1.getString("CUSTOMER_NAME")) %>","<%=exchange.unHtmls(Integer.toString(rs1.getInt("id")))%>","<%=exchange.unHtmls(rs1.getString("customer_sf"))%>","<%=moon1%>","<%=m%>")><%=demo.getLang("erp","选择")%></div>'],	
