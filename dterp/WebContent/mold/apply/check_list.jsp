...
<%
String my_sql_search = "SELECT * FROM mold_purchase_order where purchase_check_type = 0";
...
nseer_grid.column_width=[200,150,100,200,100,165];
...
--%>
['<%=rs1.getString("purchase_code")%>','<%=rs1.getString("purchase_register_time")%>','<%=rs1.getString("purchase_operater")%>','<%=rs1.getString("purchase_count")%>','<%=rs1.getString("purchase_register")%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?id=<%=exchange.unHtmls(Integer.toString(rs1.getInt("id")))%>")><%=demo.getLang("erp","审核")%></div>'],
<%--
