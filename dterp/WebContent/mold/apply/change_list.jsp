...
'<%=rs1.getString("purchase_register_time")%>',
'<%=rs1.getString("purchase_operater")%>',
'<%=rs1.getString("purchase_count")%>',
'<%=rs1.getString("purchase_register")%>',
'<%=status%>',
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("change.jsp?id=<%=exchange.unHtmls(Integer.toString(rs1.getInt("id")))%>")><%=demo.getLang("erp","变更")%></div>'],
<%--
