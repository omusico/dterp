...
String sql= "update mold_exception SET mold_status=0,exception_checker='"+realname+"',exception_check_time='"+time+"',exception_content='"+exception_content+"' WHERE id = '"+id+"'";
...
<%
id=Integer.parseInt(request.getParameter("mold_id"));
...
<form method="post" class="x-form" action="../../mold_register_picture_ex2_ok?id=<%=id%>&mold_spec=<%=mold_spec%>&mold_code=<%=mold_code%>&stock_time=<%=stock_time%>" ENCTYPE="multipart/form-data" onsubmit="return formCheck()">
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
<td <%=TD_STYLE3%> class="TD_STYLE3"><%=demo.getLang("erp","模具异常照片")%><input type="file" <%=FILE_STYLE1%> class="FILE_STYLE1" name="file1" size="70"></td>
</tr>
 </table>
</form>
</div>