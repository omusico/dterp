...
String mold_spec="";
String mold_code="";
String my_sql_search="SELECT * FROM mold_exception left join mold_info on mold_exception.mold_id=mold_info.id where mold_exception.mold_status=0 ";
...
<table width="100%" border="0" style=" text-align: right;">
  <tr>
    ...
    <input type="submit" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" onclick="productCheck()" value="<%=demo.getLang("erp","查询")%>"></td>
  </tr>
</table>
</form>
...
--%><%if(request.getParameter("mold_spec")!=null){%>
	...
	'<%=register_time%>',
	'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check_list_look.jsp?id=<%=rs1.getString("mold_exception.id")%>&mold_id=<%=rs1.getString("mold_id")%>")><%=demo.getLang("erp","审核")%></div>'],
