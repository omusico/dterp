...
String assembler="";
int i=0;
String my_sql_search="SELECT * FROM mold_info where mold_life_status=8 ";
...
nseer_grid.column_width=[200,150,150,150,150,100];
...
--%>
['<%=rs1.getString("mold_spec")%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?id=<%=rs1.getString("id")%>")><%=rs1.getString("mold_code")%></div>','<%=rs1.getString("mold_machine_number")%>','<%=rs1.getString("assembler")%>','<%=rs1.getString("assembly_time")%>'],
<%--
