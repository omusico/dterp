...
my_sql_search+=" order by mold_machine_number";
...
nseer_grid.column_width=[200,100,100,100,150,165,165];
...
--%>
['<%=rs1.getString("mold_spec")%>','<%=rs1.getString("mold_code")%>','<%=rs1.getString("mold_machine_number")%>','<%=rs1.getString("destruction_man")%>','<%=rs1.getString("destruction_time")%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("change.jsp?id=<%=rs1.getString("id")%>")><%=demo.getLang("erp","变更")%></div>'],
<%--
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("change.jsp?apply_ID=<%=rs.getString("apply_ID")%>&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'
