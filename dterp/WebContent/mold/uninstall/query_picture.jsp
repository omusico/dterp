...
<table <%=TABLE_STYLE3%> class="TABLE_STYLE3">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		...
		<div class="div_handbook"><%="您正在做的业务是：模具管理--模具拆卸管理"%></div>
		</td>
	</tr>
</table>
<%
...
String mold_spec="" ;
String mold_code="" ;
String destruction_time="" ;
if(rs.next())
{
	mold_spec=rs.getString("mold_spec") ;
	mold_code=rs.getString("mold_code") ;
	String destruction_timea=rs.getString("destruction_time") ;
	destruction_time=destruction_timea.substring(0, 10);   
}
String sql1 = "select * from mold_destruction_pic where mold_id='"+request.getParameter("id")+"'";
...
while(rs1.next())
{
	...
	path=project+":"+path;
	//path=UpLoadUrl.getHttpUrl(request);
	String url=UpLoadUrl.getHttpUrl(request);
%>
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">

 <td align=center class="TD_STYLE3">
 <img src="<%=url%>/upload_file/uninstall/uninstall_<%=mold_code%>_<%=destruction_time%>/<%=rs1.getString("pic_path")%>"> 
 </td>
 </tr>
</table>
<%}
