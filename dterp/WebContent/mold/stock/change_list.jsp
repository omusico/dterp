...
String my_sql_search="SELECT * FROM mold_info left join mold_purchase_order on mold_info.mold_purchase_id=mold_purchase_order.id where mold_info.mold_life_status='"+2+"' or mold_info.mold_life_status=1";
...
nseer_grid.auto='<%=demo.getLang("erp","变更")%>';
nseer_grid.data = [
...
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("change.jsp?id=<%=rs1.getString("mold_info.id")%>&stock_time=<%=rs1.getString("stock_time")%>")><%=demo.getLang("erp","变更")%></div>'],
<%--
