...
int i = 0;
	...
	"where mold_info.mold_life_status='2'";
...
String sql="select * from crm_file where id='"+rs1.getString("mold_purchase_supplier_id")+"'";
 ...
 %>
['<%=rs1.getString("mold_spec") %>','<%=rs1.getString("mold_code") %>','<div style="text-decoration : underline;color:#3366FF;CURSOR: hand;" onClick=addSpec("<%=exchange.unHtmls(rs1.getString("mold_spec")) %>","<%=exchange.unHtmls(rs1.getString("mold_code")) %>","<%=exchange.unHtmls(Integer.toString(rs1.getInt("mold_info.id")))%>","<%=exchange.unHtmls(stock_time) %>","<%=CUSTOMER_NAME%>","<%=exchange.unHtmls(rs1.getString("purchase_code"))%>")><%=demo.getLang("erp","选择")%></div>'],	
...
function addSpec(strSpec,strLot_no,id,stock_time,mold_purchase_supplier_id,mold_purchase_id)
{
//alert(stock_time);
