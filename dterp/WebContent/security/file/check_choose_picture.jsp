...
%>
<form method="post"
	action="../../hr_file_check_ok?human_ID=<%=human_ID%>"
	ENCTYPE="multipart/form-data">
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		<td><div>您已经审核通过，请点击完成</div></td>
		<td <%=TD_STYLE3%> class="TD_STYLE3">
		<div <%=DIV_STYLE1%> class="DIV_STYLE1">
		<%-- 
		<input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","上传照片")%>">&nbsp;
		<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","上传附件")%>" onClick=location="check_choose_attachment.jsp?human_ID=<%=human_ID%>">&nbsp;
		--%>
		<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","完成")%>" onClick=location="check_list.jsp"></div>
		</td>
	</tr>
</table>
<%-- 
