...
%>
<form method="post"
	action="../../hr_file_register_ok?human_ID=<%=human_ID%>"
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		...
		<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","上传附件")%>" onClick=location= "register_choose_attachment.jsp?human_ID=<%=human_ID%>">&nbsp;
		--%>
		</td>
		<td style="text-align:right">
		<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","完成")%>" onClick=location="register.jsp">
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		<td <%=TD_STYLE3%> class="TD_STYLE3"><%=demo.getLang("erp", "提交成功，如有照片，请选择上传照片(*.jpg、*.gif文件)")%><input
			type="file" <%=FILE_STYLE1%> class="FILE_STYLE1" <%=FILE_STYLE1%>
			class="FILE_STYLE1" name="file1"></td>
	</tr>
</table>
--%>
</div>
