...
%>
	['<%=rs.getString("hr_file.idcard")%>','<%=exchange.toHtml(rs.getString("human_name"))%>','<%=exchange.toHtml(rs.getString("name"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<%=timeShow%>','<%=exchange.toHtml(rs.getString("expiry_period"))%>','<%=exchange.toHtml(enrollment_tag)%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("change_choose.jsp?human_ID=<%=rs.getString("hr_file.human_ID")%>&&human_name=<%=toUtf8String.utf8String(exchange.toURL(rs.getString("human_name")))%>")><%=demo.getLang("erp","变更")%></div>'],
<%}
}%></page:pages>
['']];
nseer_grid.init();
