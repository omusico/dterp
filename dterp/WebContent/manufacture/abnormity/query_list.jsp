...
if(request.getParameter("product_no")!=null||request.getParameter("keyword")!=null||request.getParameter("product_name")!= null||request.getParameter("register_name")!= null){
	...
	int i =0;
//where a.event_checker  is not null and a.event_checker != '' 删除的条件
	String my_sql_search = "SELECT * FROM event_info a left join product_info b on event_product_id =b.id ";	
...
{
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?event_product_id=<%=exchange.unHtmls(Integer.toString(rs1.getInt("event_product_id")))%>")><%=demo.getLang("erp","查看")%></div>'<%}else{%>,''<%}%>],	
