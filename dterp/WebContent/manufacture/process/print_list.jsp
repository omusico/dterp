...
<%
		...
		+ my_condition + " group by p1.id,p1.product_lot_no,p1.product_type,p1.product_spec_id,p1.product_spec,p1.product_status,p1.product_pstatus,p1.product_produce_location order by p1.id";
...
<form action="print_list.jsp" method="post" name="search_form" id="search_form">
...
nseer_grid.columns =[
	                   ...
	                   {name: '<%=demo.getLang("erp","打印")%>'}
]
nseer_grid.column_width=[240,240,240,240,240];
...
'<%=plan_typeD %>',
'<%=product_specD%>',
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("print.jsp?id=<%=rs.getString("p1.id") %>")><%=demo.getLang("erp", "打印")%></div>'],
