...
//		查询
		if (request.getParameter("plan_typeS1") != null&&request.getParameter("plan_typeS1").length() > 0) {
			if(!request.getParameter("plan_typeS1").equals("all")){
				if(request.getParameter("plan_typeS1").equals("1")){
					my_condition += " and (plan_check_tag="+request.getParameter("plan_typeS1")+" or plan_check_tag=3)";
				}else{ 
					my_condition += " and plan_check_tag="+request.getParameter("plan_typeS1");
				}
			}
		}
		//查询条件加入check_tag,标示未删除
...
<table <%=TABLE_STYLE3%> class="TABLE_STYLE3">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
		...
		生产计划类型：
		  ...
		  <%		
				}else if(request.getParameter("plan_typeS1").equals("0")){
		  %>
		  <option value="all">---------全部---------</option>
