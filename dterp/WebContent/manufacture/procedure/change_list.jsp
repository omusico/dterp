...
<%
		...
		//
		String my_sql_search = "select p1.id,p1.product_lot_no,p1.product_type,p1.product_spec_id,p1.product_spec,p1.product_status,p1.product_pstatus,p1.product_produce_location ,p2.product_date "+
			"FROM product_info p1, "+
			"(SELECT p4.id,p4.father_product_id,pi.product_4_date  as product_date "+
			"FROM product_info p4, "+
			"product_4_info pi "+
			"WHERE "+ 
				"p4.id = pi.product_id "+
			") p2 "+ 
			"where  p1.id = p2.father_product_id "+
			"and (p1.product_status = 2 or p1.product_status = 3) "+
			"and (p1.product_pstatus ='' or p1.product_pstatus ='正常') "+
			"and p2.id in (select product_id from product_4_info group by product_id) "+my_condition+
			"group by p1.id,p1.product_lot_no,p1.product_type,p1.product_spec_id,p1.product_spec,p1.product_status,p1.product_pstatus,p1.product_produce_location,p2.product_date "+
			"union "+
			"select p1.id,p1.product_lot_no,p1.product_type,p1.product_spec_id,p1.product_spec,p1.product_status,p1.product_pstatus,p1.product_produce_location ,p2.product_date "+
			"FROM product_info p1, "+
			"(SELECT p8.id,p8.father_product_id,pi.product_8mm_paper_date as product_date "+
			"FROM product_info p8, "+
			     "product_8mm_info pi "+
			"WHERE "+ 
			     "p8.id = pi.product_id "+ 
			     ") p2 "+
			...
			"and (p1.product_status = 2 or p1.product_status = 3) "+ 
			"and (p1.product_pstatus ='' or p1.product_pstatus ='正常') "+ 
			"and p2.id in (select product_id from product_8mm_info group by product_id) "+my_condition+ 
			"group by p1.id,p1.product_lot_no,p1.product_type,p1.product_spec_id,p1.product_spec,p1.product_status,p1.product_pstatus,p1.product_produce_location,p2.product_date "+
			"union "+
			"select p1.id,p1.product_lot_no,p1.product_type,p1.product_spec_id,p1.product_spec,p1.product_status,p1.product_pstatus,p1.product_produce_location ,p2.product_date "+
			"FROM product_info p1, "+
			"(SELECT ph.id,ph.father_product_id,pi.product_hole_date as product_date "+
			"FROM product_info ph, "+
			     "product_hole_info pi "+
			"WHERE "+ 
			     "ph.id = pi.product_id "+
			     ") p2 "+ 
			"where  p1.id = p2.father_product_id "+ 
			"and (p1.product_status = 2 or p1.product_status = 3) "+ 
			"and (p1.product_pstatus ='' or p1.product_pstatus ='正常') "+ 
			"and p2.id in (select product_id from product_hole_info group by product_id) "+my_condition+ 
			"group by p1.id,p1.product_lot_no,p1.product_type,p1.product_spec_id,p1.product_spec,p1.product_status,p1.product_pstatus,p1.product_produce_location,p2.product_date  ";
...
nseer_grid.column_width=[120,120,120,120,120,80];
...
String product_is="";
//验证产品是否被生产
String sql_product_is="select id,product_is_production,father_product_id from product_info where father_product_id='"+rs.getString("id")+"'";
ResultSet rs_product_is = manufacture_db1.executeQuery(sql_product_is);
while(rs_product_is.next()){
	if(rs_product_is.getString("product_is_production").equals("0")){
		product_is=rs_product_is.getString("product_is_production");
		
	}
}

k++;
if(rs.getString("product_produce_location").equals("0")){
	plan_typeD="4分切";
	product_page="change_p4";
}else if(rs.getString("product_produce_location").equals("1")){
	plan_typeD="8mm切";
	product_page="change_p8";
}else if(rs.getString("product_produce_location").equals("2")){
	plan_typeD="打孔";
	product_page="change_phole";
}else if(rs.getString("product_produce_location").equals("3")){
	plan_typeD="包装";
}
if(rs.getString("product_status").equals("2")){
	product_specD="生产中";
}else if(rs.getString("product_status").equals("3")){
	product_specD="生产完成";
}
%>
...
'<%=plan_typeD %>',
'<%=product_specD%>',
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("<%=product_page%>.jsp?id=<%=rs.getString("id") %>")><%=demo.getLang("erp", "变更")%></div>'
],
