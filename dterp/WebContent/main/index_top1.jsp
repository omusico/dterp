...
<tr>
  ...
  <%String sql="";
	if(field_type.equals("0")){
	sql = "select * from document_main where value!='开发管理' order by id";
}else{
	sql = "select * from document_main where value!='开发管理' and fieldType_tag='1' order by id";
}
...
onclick="selectM(this,'../<%=table%>/include/index_middle1.jsp?mod_c=<%=demo.getLang("erp",rs.getString("value"))%>&&main_code=<%=rs.getString("main_code")%>');" 
name="nseer_mod" 
nseer1="<%=rs.getString("reason")%>" 
target="a" 
onmouseover="window.status='上海慧索计算机科技';this.href='javascript:void(0)'; return true">
<span><font color="#FFFFFF"><%=demo.getLang("erp",rs.getString("value"))%></span></a></li>
