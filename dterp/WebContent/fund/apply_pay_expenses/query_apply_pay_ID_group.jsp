...
%>
 <table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=demo.getLang("erp","费用申请单编号")%>：<font color="<%=color%>"><%=rs.getString("apply_pay_ID")%><%=apply_tag%></font> <%=demo.getLang("erp","其中审核状态")%>：<font color="<%=color1%>"><%=exchange.toHtml(check_tag)%></font> <%=demo.getLang("erp","费用状态")%>：<font color="<%=color2%>"><%=exchange.toHtml(pay_tag)%></font></td>
 </tr>
 </table>
