...
<%}else if(file_chain_name.indexOf("应付账款")!=-1){
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><a href="javascript:winopen('../../purchase/purchase/query_purchase_ID_group.jsp?purchase_ID=<%=apply_ID_group%>')"><%=exchange.toHtml(apply_ID_group)%></td>
	 <%}else{%>
	 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><a href="javascript:winopen('../../intrmanufacture/intrmanufacture/query_intrmanufacture_ID_group.jsp?intrmanufacture_ID=<%=apply_ID_group%>')"><%=exchange.toHtml(apply_ID_group)%></td>
	 <%}}else{%>
...
<%}}else if(file_chain_name.indexOf("工资")!=-1){%>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><a href="javascript:winopen('../../hr/salary/query_unit_list.jsp?salary_ID=<%=apply_ID_group%>')"><%=exchange.toHtml(apply_ID_group)%></td>
<%}else{%>
