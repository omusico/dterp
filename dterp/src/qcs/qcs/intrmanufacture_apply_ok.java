...
String sqla="update intrmanufacture_intrmanufacture set qcs_apply_tag='1' where intrmanufacture_id='"+intrmanufacture_id+"'";
...
while(rs.next()){
	   String sql="update stock_gather set qcs_dealwith_tag='1' where reasonexact='"+intrmanufacture_id+"' and reasonexact_details='"+rs.getString("provider_name")+"'";
		...
		sql="update fund_fund set qcs_dealwith_tag='0' where reasonexact='"+intrmanufacture_id+"' and funder_id='"+rs.getString("provider_id")+"'";
		qcs_db.executeUpdate(sql);
}
qcs_db1.commit();
qcs_db.commit();
