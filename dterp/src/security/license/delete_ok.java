...
public class delete_ok extends HttpServlet {
	...
	public synchronized void service(HttpServletRequest request,
		...
		try {
			if (security_db.conn((String) dbSession
...
//					sql = "delete from " + table1 + " where human_ID='"
//							+ human_ID + "'";
//					security_db.executeUpdate(sql);
				...
				sql = "delete from security_workspace where human_ID='"
						+ human_ID + "'";
				security_db.executeUpdate(sql);
				String sql2 = "update hr_file set license_tag='0' where human_ID='"
						+ human_ID + "'";
