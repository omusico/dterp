					...
					if(n==0){
						...
						try{   
							...
							for(int i=1;i<num;i++){
								...
								String tem_amount="amount"+i;
								String tem_personal_unit="personal_unit"+i;
								String tem_cost_price="cost_price"+i;
								String tem_shuilv_sum="order_sale_bonus_rate"+i;
								...
								String amount=request.getParameter(tem_amount) ;
								String personal_unit=request.getParameter(tem_personal_unit) ;
								String cost_price2=request.getParameter(tem_cost_price) ;
								String shuilv=request.getParameter(tem_shuilv_sum) ;
								...
								shuilv_sum +=Double.parseDouble(cost_price)*Double.parseDouble(shuilv);

								String sql1 = "insert into design_module_details(design_ID,details_number,product_ID,product_name,type,product_describe,amount,cost_price,amount_unit,subtotal) values ('"+design_ID+"','"+i+"','"+product_ID+"','"+product_name+"','"+type+"','"+product_describe+"','"+amount+"','"+cost_price+"','"+personal_unit+"','"+subtotal+"')" ;
								design_db.executeUpdate(sql1) ;
							}
							shuilv_sum = shuilv_sum / 100;
							List rsList = GetWorkflow.getList(design_db, "design_config_workflow", "03");
							if(rsList.size()==0){

								String sql2="update design_module set cost_price_sum='"+cost_price_sum+"',check_tag='1',c_define20='"+shuilv_sum+"' where design_ID='"+design_ID+"'";
								...
								String sql2="update design_module set cost_price_sum='"+cost_price_sum+"',c_define20='"+shuilv_sum+"' where design_ID='"+design_ID+"'";
								design_db.executeUpdate(sql2) ;
