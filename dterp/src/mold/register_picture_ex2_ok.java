...
public class register_picture_ex2_ok extends HttpServlet {// 创建方法
		...
		try {
			...
			if (mold_db.conn((String) dbSession.getAttribute("unit_db_name"))) {
				...
				try {
						...
						File b = new File(UpLoadUrl.getUploadUrl()+path + "/upload_file/mold_exception_picture/exception_pictures"+"_"+mold_code+"_"+stock_time+"/");
						if(!b.exists()){
							b.mkdirs();
						}
						//最终路径
						String all_path=UpLoadUrl.getUploadUrl()+path + "/upload_file/mold_exception_picture/exception_pictures"+"_"+mold_code+"_"+stock_time+"/" + file;
						myFile.saveAs(all_path);
						//写入数据库
						String sql = "insert into mold_exception_pic(mold_id,pic_path)values('"+id+"','"+file+"')";
						mold_db.executeUpdate(sql);
					   }
					response.sendRedirect(path+"/mold/exception/register_search2.jsp");
