...
public class register_setup_ok extends HttpServlet {
	...
	try {
			...
			if (mold_db.conn((String) dbSession.getAttribute("unit_db_name"))) {
				...
				// 插入模具详情表
				String sql = "update mold_info set mold_machine_number='"+mold_machine_number+"',top_mold_code='"+top_mold_code+"',bottom_mold_code='"+
				bottom_mold_code+"',lock_code='"+lock_code+"',installer='"+installer+"',installation_time='"+installation_time+
				"',mold_life_status='7' where id='"+id+"'";
				// 执行插入语句（模具详情表）
...
//						+"')";
				 ...
				 sql="update mold_assembly_installation set installation_item1='"+radio16+"',installation_item2='"+radio17+
				 "',installation_item3='"+radio18+"',installation_item4='"+radio19+"',installation_item5='"+radio20+
				 "',installation_item6='"+radio21+"',installation_item7='"+radio22+"',mold_installation_register='"+mold_ai_register+
				 "',mold_installation_regist_time='"+mold_ai_regist_time+"' where mold_id='"+id+"'";
				// 执行插入语句（模具组装安装表）
				mold_db.executeUpdate(sql);
				...
				// 跳转页面
				response
						.sendRedirect(path+"/mold/setup/register_search.jsp");
