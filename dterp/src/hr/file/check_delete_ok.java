...
String remark = request.getParameter("demand");//新加备注
...
if(!rs6.next()){
	String sql = "update hr_file set check_tag='2',C_DEFINE12='"+remark+"' where human_ID='"+human_ID+"'" ;
	hr_db.executeUpdate(sql) ;
	sql = "delete from hr_workflow where object_ID='"+human_ID+"' and type_id='01'" ;
		hr_db.executeUpdate(sql) ;
	}else{

	sql6="select id from hr_workflow where object_ID='"+human_ID+"' and type_id='01' and config_id<'"+config_id+"' and config_id>='"+""+"'";
	rs6=hr_db.executeQuery(sql6);
	while(rs6.next()){
		String sql = "update hr_workflow set check_tag='0',remark='"+remark+"' where id='"+rs6.getString("id")+"' and type_id='01'" ;
		hr_db1.executeUpdate(sql) ;
	}
	}	
response.sendRedirect("security/file/check_delete_ok1.jsp");
hr_db.commit();
hr_db1.commit();
hr_db.close();
hr_db1.close();
