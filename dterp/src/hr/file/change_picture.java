	...
	public synchronized void service(HttpServletRequest request,
		...
		try {
			...
			if (hr_db.conn((String) dbSession.getAttribute("unit_db_name"))) {
				...
				String zf = request.getParameter("select4");//职位分类
				String branch = request.getParameter("select5");//
				String[] zf1=zf.split("/");
				String[] branch1 = branch.split("/");
				String kindbm=request.getParameter("kind_chain");
				String[] kbm=kindbm.split(" ");
				...
				String column_group = getInsertSql.sql((String) dbSession
					...
					try {
						String sqll = "insert into hr_file_dig(" + column_group
								+ ") select " + column_group
								+ " from hr_file where human_ID='" + human_ID
								+ "'";
						hr_db.executeUpdate(sqll);
						String sql8 = "update hr_file set HUMAN_MAJOR_FIRST_KIND_ID='"+zf1[0]+"',HUMAN_MAJOR_FIRST_KIND_NAME='"+zf1[1]+"'," +
								"HUMAN_MAJOR_SECOND_KIND_ID='"+branch1[0]+"',HUMAN_MAJOR_SECOND_KIND_NAME='"+branch1[1]+"',"+
								"chain_id='"+kbm[0]+"',chain_name='"+kbm[1]+"', human_name='"
								+ human_name
								+ "',human_address='"
								+ human_address
								+ "',human_title_class='"
								...
								+ "',human_email='"
								+ human_email
								+ "',check_tag='"+check_tag+"',change_tag='1' where human_ID='"
								+ human_ID + "'";
						hr_db.executeUpdate(sql8);
						/** ************************************************** */
						CdefineUpdate CdefineUpdate = new CdefineUpdate();
						sql8 = CdefineUpdate.update("hr_file", "human_ID",
								human_ID, request);
						hr_db.executeUpdate(sql8);
						/** ************************************************** */

						String sql9 = "update security_users set human_name='"
								+ human_name + "' where human_ID='" + human_ID
								+ "'";
						hr_db.executeUpdate(sql9);
						String sql10 = "update security_license set human_name='"
								+ human_name
								+ "' where human_ID='"
								+ human_ID
								+ "'";
						...
						List rsList = GetWorkflow.getList(hr_db,
								"hr_config_workflow", "01");
						String[] elem = new String[3];
						if (rsList.size() == 0) {
							String sql = "update hr_file set check_tag='"+check_tag+"' where human_ID='"
									+ human_ID + "'";
							hr_db.executeUpdate(sql);
						} else {
							String sql = "delete from hr_workflow where object_ID='"
									+ human_ID + "' and type_id='01'";
							hr_db.executeUpdate(sql);
							sql = "update hr_file set check_tag='"+check_tag+"' where human_ID='"
									+ human_ID + "'";
							hr_db.executeUpdate(sql);
							Iterator ite = rsList.iterator();
							while (ite.hasNext()) {
								elem = (String[]) ite.next();
								sql = "insert into hr_workflow(config_id,object_ID,type_id,describe1,describe2) values ('"
										+ elem[0]
										+ "','"
										+ human_ID
										+ "','01','"
						...
						response
								.sendRedirect("security/file/change_choose_picture.jsp?human_ID="
										+ human_ID + "");
