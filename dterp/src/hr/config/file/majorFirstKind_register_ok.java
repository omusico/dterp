	...
	public synchronized void service(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException {
		...
		try {
			...
			if (hr_db.conn((String) dbSession.getAttribute("unit_db_name"))) {
				...
				String sqll = "select * from hr_config_major_first_kind where first_kind_ID='"
						+ first_kind_ID
						+ "' or first_kind_name='"
						+ first_kind_name + "'";
				ResultSet rs = hr_db.executeQuery(sqll);
				if (rs.next()) {
					response.sendRedirect("security/config/file/majorFirstKind_register_ok_a.jsp");
				} else {
					String sql = "insert into hr_config_major_first_kind(first_kind_ID,first_kind_name) values('"
							+ first_kind_ID 
							+ "','" 
							+ first_kind_name + "')";
					hr_db.executeUpdate(sql);
					String sql2 = "insert into hr_config_major_second_kind(first_kind_ID,first_kind_name,second_kind_ID) values('"
							+ first_kind_ID
							+ "','"
							+ first_kind_name
							+ "','2')";
					...
					response.sendRedirect("security/config/file/majorFirstKind_register_ok_b.jsp");
				}
				hr_db.commit();
				hr_db.close();
			} else {
