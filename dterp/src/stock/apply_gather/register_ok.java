...
*/
	...
	String sql = "insert into stock_apply_gather(gather_ID,gatherer_name,gatherer_ID,reason,register_time,demand_return_time,remark,register,check_tag,excel_tag,not_return_tag) values ('"+gather_ID+"','"+gatherer_name+"','"+gatherer_ID+"','"+reason+"','"+register_time+"','"+demand_return_time+"','"+remark+"','"+register+"','0','2','"+not_return_tag+"')" ;
...
//**********************************************************
if(rsList.size()==0){
	String nseer_sql="update stock_apply_gather set cost_price_sum='"+cost_price_sum+"',demand_amount='"+demand_amount+"',check_tag='1' where gather_ID='"+gather_ID+"'";
...
if(!vr.validata((String)dbSession.getAttribute("unit_db_name"),"stock_pay","reasonexact",gather_ID)){
	...
	stock_db.executeUpdate(sql5) ;
		...
		String nseer_sql="update stock_apply_gather set cost_price_sum='"+cost_price_sum+"',demand_amount='"+demand_amount+"',check_tag='0' where gather_ID='"+gather_ID+"'";
		stock_db.executeUpdate(nseer_sql);
...
response.sendRedirect("stock/apply_gather/register_ok_a.jsp");
}else{
response.sendRedirect("stock/apply_gather/register_ok_b.jsp");
}
stock_db.commit();
stock_db.close();
}else{
