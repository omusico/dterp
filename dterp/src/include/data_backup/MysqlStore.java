 ...
 * MysqlStore.java
 * 
 * 用于mysql在windows上的数据备份和恢复 需要配置mysql.prorperties文件的用户名、密码、路径等信息。 当程序用在普通的java
 * application时,mysql.properties文件位 于MysqlStore.class下级的conf中；当用web
 * application时以 JSP服务器不同位置不同，例如在resin时，文件放到resin\conf下
 * 
...
public class MysqlStore {
	...
	public boolean backup(String fileName, String database) {
		...
		try {
			...
			/**
			 * mysql5.0.18做了相应的修改 mysqldump -x -u"+userName+" -p"+password+" -c
			 * --default-character-set=UTF-8 --databases ondemand2
			 */
			
			String strBackup = "";
			if("".equals(password)){
				strBackup = "mysqldump -x -u" + userName /*+ " -p"
				+ password*/
			...
			Process process = rt.exec(strBackup);// 因为两个进程难以同步，该dos命令缺少输出到目标文件，所以采用下面的解决方法从缓存得到，写入目标文件
			String line = "";
			BufferedWriter bufferedWriter = new BufferedWriter(
					new OutputStreamWriter(new FileOutputStream(fileName),
							"UTF-8"));// 创建写数据流，准备接收上面dos命令的输出
			BufferedReader bufferedReader = new BufferedReader(
					new InputStreamReader(process.getInputStream(), "UTF-8"));// 读process执行的输出流，这个输出流存放在哪里，
	...
	/**
	 * 是backup的辅助方法， 根据当前系统时间与设置的文件名生成准确的备份文件名
	 * 
	 * @fileName 初始的文件名
	 */
