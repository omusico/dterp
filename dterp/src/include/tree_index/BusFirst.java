...
public class BusFirst {
	public String businessComment(String url,String base,String table_name,String field_name,String field_name1,String unit_db_name){
		String ab="";
		try {
		...
		ResultSet rs=db.executeQuery("select chain_name,hreflink from "+table+" where file_path='"+filepath+"'");
		while(rs.next()) {
		...
		rs=db.executeQuery("select used_tag from erpplatform_config_public_char where kind='nseer_file_1' and type_name='js' and used_tag='0' and all_tag='0'");
		String ad="";
		if(rs.next()){
			ad="<script type=\"text/javascript\" src=\""+url_temp+"javascript/include/nseer_cookie/ad.js\"></script>";
		}
		String other_str="";
		...
		String tds=msg_td+pdf_td+excel_import_td+excel_export_td+xml_import_td+xml_export_td+advance_search_td;
		ab=ab+"<script type=\"text/javascript\" src=\""+url_temp+"javascript/include/nseer_cookie/toolTip.js\"></script><script type=\"text/javascript\" src=\""+url_temp+"javascript/include/div/alert.js\"></script>"+ad;
		//ab+="<script>function _killerrors(){return true;}window.onerror=_killerrors;</script>";
		ab=ab+" <div style=\"position:absolute;top:25px;width:50px;left:0;\"><table><tr>"+tds+"</tr></table></div>";
